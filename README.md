...
目前測試期間，Actions 使用 `--ai-no-run-limit` **停用單次金額上限**，本流程 UTC 月額為 **US$5**；所有候選都有結果才發布。
2026-09-21 首輪曾經使用者同意暫用 US$10，現已審完 188 題並發布 141 題（47 題排除、移除 193 個歧義誘答）。本月含校驗、前期測試及失敗預留的保守估算為 US$6.0200。首輪完成紀錄已保存，程式自動恢復 US$5，workflow 也移除臨時加額參數，避免未來快取遺失時再次開放加額。
累計用量不歸零；若已超過 US$5，本月仍可沿用快取，但新增付費審查等下個 UTC 月續跑。CLI 的 `--ai-initial-month-budget` 只供另行核准的首輪使用：中斷、待審或失敗保留加額供續跑，成功審完後自動恢復 `--ai-month-budget`。OpenAI 平台額度需另外手動調整，程式不會修改平台設定。
